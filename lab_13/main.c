...
/**
 * Информация содержащаяся в текстовом файле: марка автомобиля;страна производитель;цена;срок гарантии
 * Длина строки, содержащей марку или страну не должна превышать 20 символов
 * Функции программы: загрузка данных из файла, добавление элемента, удаление элемента, изменение элемента, вывод отсортированной информации,
 * вывод выборочной информации, сохранение текущих данных в указанный файл и выход.
 * Сортировка ведется по модели, цене, по цене и сроку гарантии.
 * Фильтрация: страна производитель - Германия, срок гарантии не меньше 4 лет.
 */
            ...
            if (*end == '\0' && choice > 0 && choice < 10)
            {
                ...
                if (choice == 3)
                {
                    ...
                    if (scanf("%s", buf) == 1)
                    {
                        ...
                        if (*end == '\0' && id > 0)
                        {
                            head = delete_element(head, id, &rc);
                            if (rc == 0)
                                printf("List is empty\n");
                            else if (rc == 1)
                                printf("Deleted!\n");
                            else
                                printf("Didn't find element with id = %d\n", id);
                        }
                        else
                            printf("Incorrect id.\n");
                    }
